...
(async()=>{
 ...
 try{
  for(const width of [375,1440]){
   for(const count of [0,1,4]){
    ...
    if(count>1){
     ...
     assert.equal(await page.locator('.portfolio-slide.is-prev').count(),1);
     assert.equal(await page.locator('.portfolio-slide.is-next').count(),1);
     const sideFilter=await page.locator('.portfolio-slide.is-next').evaluate(e=>getComputedStyle(e).filter);
     assert.notEqual(sideFilter,'none');
     ...
     await page.waitForFunction(()=>document.querySelector('.portfolio-slide.is-active').getAttribute('aria-label')==='3 de 4');
     assert.equal(await active(),'3 de 4');
     await page.locator('[data-carousel-next]').click();
     await page.waitForFunction(()=>document.querySelector('.portfolio-slide.is-active').getAttribute('aria-label')==='4 de 4');
    }
  ...
  console.log('PASS: 0/1/4 photos, mobile/desktop, buttons, keyboard, dots, stacked carousel, pause and reduced motion.');
